...
class Disasm_Hopper(Disasm):
    ...
    def _replace_symbol(self, symbol, rd):
        res = symbol
    ...
    def _replace_to_times_elements(self, content, rd):
        res = content
        c_split = content.split('*')
        for i, c in enumerate(c_split):
            c_split[i] = self._replace_symbol(c, rd)
        res = '*'.join(c_split)
        return res

    def _replace_each_symbol(self, content, rd):
        res = content
        if '+' in content:
            c_split = content.split('+')
            for idx, elem in enumerate(c_split):
                if '*' in elem:
                    c_split[idx] = self._replace_to_times_elements(elem, rd)
                else:
                    c_split[idx] = self._replace_symbol(elem, rd)
            res = '+'.join(c_split)
        elif '-' in content:
            c_split = content.split('-')
            for idx, elem in enumerate(c_split):
            ...
            res = self._replace_to_times_elements(content, rd)
        else:
            res = self._replace_symbol(content, rd)
        return res

    def _replace_symbol_with_value(self, address, inst_name, arg, rd):
        res = arg
        if arg.endswith(']'):
            arg_split = arg.split('[', 1)
            prefix = arg_split[0].strip()
            mem_addr = arg_split[1].strip().rsplit(']', 1)[0].strip()
            mem_addr = self._replace_each_symbol(mem_addr, rd)
            res = prefix + ' [' + mem_addr + ']'
        else:
            res = self._replace_each_symbol(arg, rd)
        return res
